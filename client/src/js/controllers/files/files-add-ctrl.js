...
angular.module('RDash').controller('FilesAddCtrl', ['$scope', '$cookieStore', 'filesApiService', FilesAddCtrl]);

function FilesAddCtrl($scope, $cookieStore, filesApiService) {

    $scope.sourceLink = 'http://www.placehold.it/28X28/DDDDDD/AAAAAA';
    ...
    filesApiService.getVerifyUser().then((data)=> {
    ...
    $scope.addFile = ()=> {
        filesApiService.addFiles({
            uid: $scope.uid  ,
            intro: $scope.intro,
            title: $scope.title,
            doc_icon:$scope.sourceLink,
            url: $scope.url,
