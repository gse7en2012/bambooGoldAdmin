...
const $liveSelfHelpers = {
    getLiveOpinionDetails(item){
        const r = {
            allow_see_lv: item.live_user.allow_see_lv,
            ...
            status:item.status,
            writer: {
                nickname: item.live_user.nickname,
                picture: item.live_user.picture,
                uid: item.live_user.uid,
                verify: item.live_user.verify
            }
        };
        if (r.retweeted) {
            return DataBaseModel.LiveOpinion.find({
                ...
                include: [{
                    model: DataBaseModel.LiveUsers,
                    attributes: ['allow_see_lv', 'nickname', 'picture', 'uid', 'verify']
                    //required: true,
                ...
                r.quote_viewpoint = {
                    ...
                    writer: {
                        nickname: opinion.live_user.nickname,
                        uid: opinion.live_user.uid
                    }
                };
                return r;
            });
...
const LiveOpinionController = {
    ...
    addLiveOpinion(opts){
        return DataBaseModel.LiveOpinion.create({
            uid:opts.uid,
